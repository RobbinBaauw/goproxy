...
func write(packetId int, data []byte, conn *ClientConnection) {
	...
	fmt.Println("Writing packet id: ", readVarInt(&ByteArrayReader{packetIdBytes, 0}))

	length := len(packetIdBytes) + len(data)
	lengthBytes := writeVarInt(length)
	fmt.Println("Writing length: ", readVarInt(&ByteArrayReader{lengthBytes, 0}))
